...
//namespace Opencart\Admin\Controller\Extension\UltimateRestrictions\Module;
//class UltimateRestrictions extends \Opencart\System\Engine\Controller {

class ControllerExtensionModuleUltimateRestrictions extends Controller {
	...
	public function index() {
		...
		if (version_compare(VERSION, '4.0', '>=')) {
			...
			if (empty($event)) {
				$triggers = array(
					'catalog/controller/checkout/cart/before'					=> 'extension/' . $this->name . '/' . $this->type . '/' . $this->name . '|checkout',
					'catalog/controller/checkout/checkout/before'				=> 'extension/' . $this->name . '/' . $this->type . '/' . $this->name . '|checkout',
					'catalog/controller/checkout/confirm/before'				=> 'extension/' . $this->name . '/' . $this->type . '/' . $this->name . '|checkout',
					'catalog/model/design/layout/getModules/after'				=> 'extension/' . $this->name . '/' . $this->type . '/' . $this->name,
					'catalog/model/checkout/shipping_method/getMethods/after'	=> 'extension/' . $this->name . '/' . $this->type . '/' . $this->name . '|rates',
					'catalog/model/setting/extension/getExtensionsByType/after'	=> 'extension/' . $this->name . '/' . $this->type . '/' . $this->name,
				);
		...
		$data['rule_options'] = array(
			...
			'discount_criteria'			=> array('coupon', 'gift_voucher', 'ignore_specials', 'reward_points'),
			'location_criteria'			=> array('address', 'city', 'country', 'distance', 'geo_zone', 'location_comparison', 'origin', 'postcode', 'zone'),
			...
			'product_criteria'			=> array('attribute', 'attribute_group', 'category', 'filter', 'manufacturer', 'option', 'other_product_data', 'product', 'product_group', 'quantity_of_product', 'quantity_of_group', 'recurring_profile'),
		...
		foreach (array('prediscounted', 'nondiscounted', 'ignoreoptions', 'onlyoptions', 'taxed', 'total') as $total_value) {
		...
		$data['typeaheads'] = array('attribute', 'attribute_group', 'category', 'filter', 'manufacturer', 'option', 'option_value', 'product');
		...
		if (version_compare(VERSION, '2.0', '<')) {
			// recurring profiles do not exist
			...
			foreach ($this->model_catalog_subscription_plan->getSubscriptionPlans() as $plan) {
				$data['recurring_profile_array'][$plan['subscription_plan_id']] = $plan['name'];
			}
		}
		...
		foreach (array('total', 'payment', 'shipping') as $extension_type) {
			...
			foreach ($extension_query->rows as $extension) {
				...
				if (empty($extension_language['heading_title'])) $extension_language['heading_title'] = $extension['code'];
				
				$prefix = (version_compare(VERSION, '3.0', '<')) ? '' : $extension_type . '_';
				if ($this->config->get($prefix . $extension['code'] . '_status') || $extension_type == 'module') {
					...
					$data[$extension_type . '_extension_array'][$extension['code']] = '<em>' . $extension_language['heading_title'] . '</em>';
				}
			}
		}
		
		// Modules
		$data['module_extension_array'] = array();
		
		$code = '';
		$modules = $this->db->query("SELECT * FROM " . DB_PREFIX . "extension WHERE `type` = 'module' ORDER BY `code` ASC")->rows;
		
		foreach ($modules as $module) {
			if (version_compare(VERSION, '2.3', '<')) {
				$file_exists = file_exists(DIR_CATALOG . 'controller/module/' . $module['code'] . '.php');
			} elseif (version_compare(VERSION, '4.0', '<')) {
				$file_exists = file_exists(DIR_CATALOG . 'controller/extension/module/' . $module['code'] . '.php');
			} elseif (defined('DIR_EXTENSION')) {
				$file_exists = file_exists(DIR_EXTENSION . $module['extension'] . '/catalog/controller/module/' . $module['code'] . '.php');
			}
			
			$skip = array($this->name, 'restrict_checkout', 'restrict_payment', 'restrict_shipping', 'redirect_manager', 'rewrite_manager', 'smartsearch');
			if (in_array($module['code'], $skip) || !$file_exists) {
				continue;
			}
			
			$module_language = $this->loadLanguage('module/' . $module['code']);
			$module_name = $module_language['heading_title'];
			$module_settings = array();
			
			if (version_compare(VERSION, '2.0.1', '<')) {
				$module_instances = $this->config->get($module['code'] . '_module');
				if (!is_array($module_instances)) continue;
				
				foreach ($module_instances as $module_number => $module_instance) {
					$data['module_extension_array'][$module['code']] = $module_name;
					$module_settings[$module['code']] = $module_instance;
				}
			} else {
				if (version_compare(VERSION, '4.0', '>=')) {
					$module['code'] = $module['extension'] . '.' . $module['code'];
				}
				
				$module_instances = $this->db->query("SELECT * FROM `" . DB_PREFIX . "module` WHERE `code` = '" . $module['code'] . "'")->rows;
				
				if ($module_instances) {
					foreach ($module_instances as $module_instance) {
						$module_id = $module_instance['code'] . '.' . $module_instance['module_id'];
						$data['module_extension_array'][$module_id] = $module_name . ' &gt; ' . (!empty($module_instance['name']) ? $module_instance['name'] : $data['text_no_name']);
					
						$module_settings[$module_id] = (version_compare(VERSION, '2.1', '<')) ? unserialize($module_instance['setting']) : json_decode($module_instance['setting'], true);
					}
				} else {
					$module_id = $module['code'];
					$data['module_extension_array'][$module_id] = $module_name;
					
					$this->load->model('setting/setting');
					$module_settings[$module_id] = $this->model_setting_setting->getSetting($module['code']);
				}
			}
		}
		
		asort($data['module_extension_array']);
		unset($module_id);
		...
		$data['settings'][] = array(
			'key'		=> $table,
			'type'		=> 'table_start',
			'columns'	=> array('action', 'name', 'type', 'rules'),
		);
		foreach ($this->getTableRowNumbers($data, $table, $sortby) as $num => $rules) {
			...
			$data['settings'][] = array(
				...
				'options'	=> array(
					'checkout'	=> $data['text_checkout'],
					'module'	=> $data['text_modules'],
					'total'		=> $data['text_order_totals'],
					'payment'	=> $data['text_payment_methods'],
					'shipping'	=> $data['text_shipping_methods'],
				),
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'checkout_message',
				'type'		=> 'textarea',
				'class'		=> 'restricted-extensions',
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'module_extensions',
				'type'		=> 'checkboxes',
				'class'		=> 'restricted-extensions',
				'options'	=> $data['module_extension_array'],
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'total_extensions',
				'type'		=> 'checkboxes',
				'class'		=> 'restricted-extensions',
				'options'	=> $data['total_extension_array'],
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'payment_extensions',
				'type'		=> 'checkboxes',
				'class'		=> 'restricted-extensions',
				'options'	=> $data['payment_extension_array'],
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'shipping_extensions',
				'type'		=> 'checkboxes',
				'class'		=> 'restricted-extensions',
		...
		$data['settings'][] = array(
			...
			'content'	=> '
				<script>
					$(document).on("change", ".restriction-type", function(){
						$(this).parent().parent().find(".restricted-extensions").parent().hide();
						$(this).parent().parent().find(".restricted-extensions[name*=" + $(this).val() + "]").parent().show();
					}).ready(function(){
